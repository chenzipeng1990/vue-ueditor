<template>
    <div :id="randomId" name="ueditor-content" style="width: 100%;" type="text/plain"></div>
</template>

<script>

    import * as PC from '../../projectCommon';
    ...
    export default {
        ...
        props: {
            ...
            ueditorPath: {
                // UEditor 代码的路径
                type: String,
                default: PC.RESTFUL_URL + '/app/static/ueditor/'
            },
            ueditorConfig: {
                // UEditor 配置项
                type: Object,
        ...
        watch: {
            //监听 value变化,赋值到ueditor
            'value': function () {
                if (this.instance != undefined && this.content != this.value) {
        ...
        methods: {
            ...
            initEditor () {
                ...
                if (this.scriptTagStatus === 2 && this.instance === null) {
                    ...
                    this.$nextTick(() => {
                        vue.instance = window.UE.getEditor(this.randomId, this.ueditorConfig);
                        // 绑定事件，当 UEditor 初始化完成后，将编辑器实例通过自定义的 ready 事件交出去
                        vue.instance.addListener('ready', () => {
                            this.$emit('ready', vue.instance);
                            //加载内容
                            this.instance.setContent(vue.content || '');
                        });

                        vue.instance.addListener('contentChange', () => {
                            vue.content = vue.instance.getContent();
                        })
                    });
                }
            }
